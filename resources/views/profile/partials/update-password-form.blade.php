<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            {{ __('Assure-toi que ton compte utilise un mot de passe long et aléatoire pour rester sécurisé.') }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'password-updated')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="text-sm text-sonar-dark">{{ __('Enregistré.') }}</p>
            @endif
        </div>
    </form>
</section>
            ...
            <div class="mx-auto grid max-w-md grid-cols-5 items-end gap-1 px-2 pb-[calc(env(safe-area-inset-bottom)+0.45rem)] pt-2">
                ...
                @if (auth()->user()->isCitoyen())
                    <a href="{{ route('declarations.index') }}"
                       class="flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-lg px-1 text-[11px] font-medium transition {{ request()->routeIs('declarations.index') || request()->routeIs('declarations.show') ? 'text-white bg-alerte/15' : 'text-argent/60' }}">
                        <x-icon name="list" class="h-5 w-5" />
                        <span>Suivi</span>
                    </a>

                    <a href="{{ route('declarations.create') }}"
                       class="-mt-5 flex min-h-[64px] flex-col items-center justify-center gap-1 rounded-xl bg-alerte px-1 text-[11px] font-semibold text-white shadow-lg shadow-alerte/30 transition {{ request()->routeIs('declarations.create') ? 'ring-2 ring-white/70' : '' }}">
                    ...
                    <a href="{{ route('notifications.index') }}"
                       class="relative flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-lg px-1 text-[11px] font-medium transition {{ request()->routeIs('notifications.*') ? 'text-white bg-alerte/15' : 'text-argent/60' }}">
                        ...
                        <span>Alertes</span>
                        @if ($notificationsNonLues > 0)
                            <span class="absolute right-4 top-1 min-w-[16px] rounded-full bg-alerte px-1 text-center text-[10px] font-bold leading-4 text-white">
                                {{ $notificationsNonLues > 9 ? '9+' : $notificationsNonLues }}
                            </span>
                        @endif
                    </a>
                ...
                @else
                    <a href="{{ route('moderation.index') }}"
                       class="relative flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-lg px-1 text-[11px] font-medium transition {{ request()->routeIs('moderation.*') ? 'text-white bg-alerte/15' : 'text-argent/60' }}">
                        <x-icon name="shield-check" class="h-5 w-5" />
                        <span>Modérer</span>
                        @if ($declarationsEnAttente > 0)
                            <span class="absolute right-4 top-1 min-w-[16px] rounded-full bg-alerte px-1 text-center text-[10px] font-bold leading-4 text-white">
                    ...
                    <a href="{{ route('admin.users.index') }}"
                       class="flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-lg px-1 text-[11px] font-medium transition {{ request()->routeIs('admin.users.*') ? 'text-white bg-alerte/15' : 'text-argent/60' }}">
                ...
                <a href="{{ route('profile.edit') }}"
                   class="flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-lg px-1 text-[11px] font-medium transition {{ request()->routeIs('profile.edit') ? 'text-white bg-alerte/15' : 'text-argent/60' }}">
